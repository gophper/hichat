	...
	export default {
		...
		methods: {
			...
			getSocket(){
				...
				{
					this.ws = new WebSocket("ws://192.168.1.105:8889");
			...
			updateGmsgList(){
				let that = this;
				axios({
					'url': Vue.apiUrl+'/groupmsg/list?iGroupId='+that.iGroupId,
					'method': 'get',
			...
			fetchInfo(){
				let that = this;
				axios({
					'url': Vue.apiUrl+'/group/detail?iGroupId='+that.iGroupId,
					'method': 'get',
					'headers':{ 'x-requested-with': 'XMLHttpRequest'}
				}).then(function (response) {
					if(response.data.ret !== 0 && response.data.msg){
		...
		mounted(){
		    this.iGroupId = this.$route.params.uid;
			this.getSocket();
			this.fetchInfo();
			this.updateGmsgList();
		},destroyed(){
